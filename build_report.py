...
def build_pdf(filename="CU_AI_Advisor_MCA_Final_Report_Manik_Bhola.pdf", page_mappings=None):
    ...
    p_headers = [Paragraph("<b>Column Name</b>", table_header_style), 
                 ...
                 Paragraph("<b>Description</b>", table_header_style)]
    p_rows = [
        [Paragraph("id", table_cell_style), Paragraph("VARCHAR(50)", table_cell_style), Paragraph("PRIMARY KEY, NOT NULL", table_cell_style), Paragraph("Unique configuration topic key (e.g. 'attendance', 'grading')", table_cell_style)],
        [Paragraph("topic", table_cell_style), Paragraph("VARCHAR(100)", table_cell_style), Paragraph("NOT NULL", table_cell_style), Paragraph("Academic topic title descriptive name", table_cell_style)],
        [Paragraph("content", table_cell_style), Paragraph("TEXT", table_cell_style), Paragraph("NOT NULL", table_cell_style), Paragraph("Official legal policy text description as set by academic council", table_cell_style)]
    ]
    ...
    il_rows = [
        ...
        [Paragraph("username", table_cell_style), Paragraph("VARCHAR(100)", table_cell_style), Paragraph("FOREIGN KEY -> users.username", table_cell_style), Paragraph("Student account name, null if anonymous session", table_cell_style)],
        [Paragraph("user_message", table_cell_style), Paragraph("TEXT", table_cell_style), Paragraph("NOT NULL", table_cell_style), Paragraph("Input text query sent by the user", table_cell_style)],
        ...
        [Paragraph("routing_mode", table_cell_style), Paragraph("VARCHAR(50)", table_cell_style), Paragraph("None", table_cell_style), Paragraph("Query resolution source ('local_match' or 'puter_ai')", table_cell_style)],
        [Paragraph("latency_ms", table_cell_style), Paragraph("FLOAT", table_cell_style), Paragraph("None", table_cell_style), Paragraph("Elapsed network processing time in milliseconds", table_cell_style)],
        [Paragraph("sentiment_score", table_cell_style), Paragraph("FLOAT", table_cell_style), Paragraph("None", table_cell_style), Paragraph("Sentiment polarity score value generated by TextBlob (-1.0 to 1.0)", table_cell_style)],
    ...
    ap_rows = [
        ...
        [Paragraph("student_name", table_cell_style), Paragraph("VARCHAR(100)", table_cell_style), Paragraph("FOREIGN KEY -> users.username", table_cell_style), Paragraph("Account username booking the session slot", table_cell_style)],
        [Paragraph("advisor_name", table_cell_style), Paragraph("VARCHAR(100)", table_cell_style), Paragraph("NOT NULL", table_cell_style), Paragraph("Name of target advisor booked (e.g. Kashish Gupta)", table_cell_style)],
        [Paragraph("booking_date", table_cell_style), Paragraph("DATE", table_cell_style), Paragraph("NOT NULL", table_cell_style), Paragraph("Selected date for the advising appointment slot", table_cell_style)],
        [Paragraph("time_slot", table_cell_style), Paragraph("VARCHAR(50)", table_cell_style), Paragraph("NOT NULL", table_cell_style), Paragraph("Target booking time range (e.g. 10:00 AM - 10:30 AM)", table_cell_style)],
        [Paragraph("topic", table_cell_style), Paragraph("VARCHAR(255)", table_cell_style), Paragraph("NOT NULL", table_cell_style), Paragraph("Student-specified description of consultation reasons", table_cell_style)],
        [Paragraph("status", table_cell_style), Paragraph("VARCHAR(50)", table_cell_style), Paragraph("DEFAULT 'Scheduled'", table_cell_style), Paragraph("Current meeting status ('Scheduled' or 'Cancelled')", table_cell_style)]
    ]
